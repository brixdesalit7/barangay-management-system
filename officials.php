...
<body>
          ...
          <div class="col-sm-10 p-0">
              ...
              <div class="container-fluid">
                  <div class="btn mt-5 d-sm-flex justify-content-between">  
                          <div class="header">
                            <h1 class="text-danger"><i class="fas fa-users px-3"></i>Barangay Officials</h1>
                          </div>
                          <div class="button">
                          <input type="text" placeholder="Search" id="myInput" class="form-control mb-3">
                          <button data-bs-toggle="modal" data-bs-target="#addrecord" class="btn btn-danger btn-lg"><i class="fas fa-plus"></i>Add Appointed Official</button> 
                    ...
                    <table class="table table-striped text-center">
                      ...
                      <!-- DISPLAY DATA FROM DATABASE -->
                    	<?php
                            ...
                            $query=mysqli_query($conn,"select * from `tblofficial`");
                            while($row=mysqli_fetch_array($query)){
                              ?>
                              <tr">
                                <td><?php echo ($row['Name']); ?></td>
                                <td><?php echo ($row['Address']); ?></td>
                                <td><?php echo ($row['StartofTerm']); ?></td>
                                <td><?php echo ($row['EndofTerm']); ?></td>
                                <td><?php echo ($row['Position']); ?></td>
                                <td>
                                <button data-bs-toggle="modal" data-bs-target="#edit<?php echo $row['id'];?>" class="btn btn-warning"><i class="fas fa-edit"></i>Update</button> 
                                <button data-bs-toggle="modal" data-bs-target="#del-official<?php echo $row['id']; ?>" class="btn btn-danger"><i class="fas fa-trash-alt"></i>Delete</button> 
                                <button data-bs-toggle="modal" data-bs-target="#view-official<?php echo $row['id']; ?>" class="btn btn-primary"><i class="fas fa-eye"></i>View</button> 
                                <?php include('edit-del-official.php'); ?>
                              </td>
    ...
    <script>
        function Export() {
            ...
            if ( conf == true) {
                window.open("export-official.php",'_blank');
            } else if(conf == false ) {
              window.close();
            }
        }
